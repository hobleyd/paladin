...
// App-owned durable persistence for desktop_updater 3.x's pending native
// install recovery marker (adapted from desktop_updater's own example
// JsonFileUpdateRecoveryStore - the package ships only the abstract
// UpdateRecoveryStore interface, not a default implementation).
final class PaladinUpdateRecoveryStore implements UpdateRecoveryStore {
  PaladinUpdateRecoveryStore(this.file);
  ...
  @override
  Future<UpdateInstallRecoveryMarker?> readPendingInstall({required String channel}) async {
  ...
  Future<void> writePendingInstall(UpdateInstallRecoveryMarker marker) async {
    ...
    await pending.writeAsString('${jsonEncode(_markerToJson(marker))}\n', flush: true);
